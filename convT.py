...
'''
This code corresponds to Section 6.1 (Figure 4) of the [arXiv preprint](https://arxiv.org/abs/2504.19250)

Tested with NGSolve version 6.2.2404

t-Convergence test for fully transient poro-viscoelastic problem
...
def collecterrors(k, h, maxr, tend):
    l2e_s = []
    l2e_u = []
...
def hconvergenctauEble(e_1, e_2, maxr):
    print("===============================================================")
    print(" Mesh   Errors_s    Order   Error_u   Order")
    print("---------------------------------------------------------------")
    rate1 = []
    rate2 = []
    for i in range(maxr):
    ...
    for i in range(maxr):
        print(" 1/%-4d %8.2e   %s   %8.2e   %s  " %
              (2**(i + 1), e_1[i], rate1[i],
               e_2[i], rate2[i]))  # Include the third error and rate
...
maxlevels = 10 # Levels of mesh refinement
pp = 3 # Polynomial degree
tend = 0.5 # Final time
h = 1/16


er_s, er_u = collecterrors(pp, h, maxlevels, tend)
hconvergenctauEble(er_s, er_u, maxlevels)